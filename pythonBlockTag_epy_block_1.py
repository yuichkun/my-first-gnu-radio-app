    ...
    def work(self, input_items, output_items):
        ...
        for tag in tag_tuple:
            if pmt.to_python(tag.key) == 'detect':
                relative_offset_list.append(tag.offset - self.nitems_read(0))

        relative_offset_list.sort()

        for index in range(len(output_items[0])):
            output_items[0][index] = self.samples_since_detection

            if (len(relative_offset_list) > 0 and index >= relative_offset_list[0]):
                relative_offset_list.pop(0)
                self.samples_since_detection = 0
            else:
                self.samples_since_detection = self.samples_since_detection + 1
